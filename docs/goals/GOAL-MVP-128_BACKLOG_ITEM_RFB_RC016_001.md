# GOAL-MVP-128 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-128_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Expose required folded-state screenshot sha256 map in archive evidence contract so reviewers can verify exact screenshot artifact identity offline.
```
...
goal_card=docs/goals/GOAL-MVP-128_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_128_BACKLOG_ITEM_RFB_RC016_001_2026_05_08.md
```
...
GOAL-MVP-127 implementation commit 27e723f
```
...
```text
small package evidence-contract hardening for required folded screenshot sha256 map
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-128_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded_state_screenshot_sha256 contract
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to screenshot path->sha256 evidence
```
...
commit message: feat(secupilot): GOAL-MVP-128 folded screenshot sha256 map
