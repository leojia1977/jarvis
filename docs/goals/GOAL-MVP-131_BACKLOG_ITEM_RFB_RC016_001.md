# GOAL-MVP-131 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-131_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Add explicit expected-section metadata (AI advice source) into folded-state archive evidence so reviewer checks can deterministically map the screenshot to the intended UI section.
```
...
goal_card=docs/goals/GOAL-MVP-131_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_131_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-130 implementation commit a78a611
```
...
```text
small package evidence-contract hardening for folded-state expected-section metadata
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-131_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded-state expected-section metadata
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to explicit expected-section evidence
```
...
commit message: feat(secupilot): GOAL-MVP-131 folded screenshot expected section
