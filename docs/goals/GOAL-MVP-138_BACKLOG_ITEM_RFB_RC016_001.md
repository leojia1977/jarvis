# GOAL-MVP-138 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-138_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Version the folded-state screenshot archive evidence object so downstream validation can enforce a stable schema contract for first-load folded proof.
```
...
goal_card=docs/goals/GOAL-MVP-138_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_138_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-137 implementation commit 346d531
```
...
```text
small package evidence-contract hardening for folded-state archive evidence versioning
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-138_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded screenshot archive_evidence carries stable schema version marker
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to evidence schema-version hardening
```
...
commit message: feat(secupilot): GOAL-MVP-138 version folded evidence schema
