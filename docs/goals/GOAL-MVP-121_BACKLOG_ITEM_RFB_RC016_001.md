# GOAL-MVP-121 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-121_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Enforce one first-load folded-state AI advice source screenshot as mandatory archive evidence in every local/offline RC review package build.
```
...
goal_card=docs/goals/GOAL-MVP-121_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_121_BACKLOG_ITEM_RFB_RC016_001_2026_05_08.md
```
...
```text
single-package-builder rule hardening for folded-state screenshot archive evidence
matching unittest updates for required screenshot and HOLD behavior
reviewer-facing checklist/start-here wording update inside the builder output templates
```
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-121_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output including folded screenshot required behavior
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to mandatory folded screenshot archive evidence
```
...
commit message: feat(secupilot): GOAL-MVP-121 enforce folded screenshot evidence
