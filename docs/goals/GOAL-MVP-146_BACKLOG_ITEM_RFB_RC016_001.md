# GOAL-MVP-146 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-146_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Bind folded-state archive evidence to the exact backlog item id so package validation can trace proof metadata directly to RFB-RC016-001.
```
...
goal_card=docs/goals/GOAL-MVP-146_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_146_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-145 implementation commit c5fdb40
```
...
```text
small package evidence-contract hardening for folded-state backlog item id metadata
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-146_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded screenshot backlog item id metadata exists in package manifest/package index pathways
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to backlog id traceability hardening
```
...
commit message: feat(secupilot): GOAL-MVP-146 folded screenshot backlog item id
