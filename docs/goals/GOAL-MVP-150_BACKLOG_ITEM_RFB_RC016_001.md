# GOAL-MVP-150 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-150_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Add folded-state review source-priority metadata so archive evidence preserves the RC016 feedback priority used for this screenshot requirement.
```
...
goal_card=docs/goals/GOAL-MVP-150_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_150_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
```text
small package evidence-contract hardening for folded-state review source-priority traceability
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-150_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded-state review source priority metadata exists in package manifest/package index/screenshot index pathways
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to source-priority traceability hardening
```
...
commit message: feat(secupilot): GOAL-MVP-150 folded screenshot source priority
