# GOAL-MVP-126 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-126_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Expose a single canonical folded-state screenshot path in archive evidence contract to remove ambiguity for offline validators.
```
...
goal_card=docs/goals/GOAL-MVP-126_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_126_BACKLOG_ITEM_RFB_RC016_001_2026_05_08.md
```
...
GOAL-MVP-125 implementation commit ee22f56
```
...
```text
small archive-evidence contract clarification for canonical folded-state screenshot path
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-126_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded_state_primary_screenshot contract
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to canonical screenshot path contract
```
...
commit message: feat(secupilot): GOAL-MVP-126 canonical folded screenshot path
