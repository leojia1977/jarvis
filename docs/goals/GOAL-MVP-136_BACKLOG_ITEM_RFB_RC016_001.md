# GOAL-MVP-136 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-136_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Extend folded-state screenshot archive evidence so SCREENSHOT_INDEX entries explicitly carry FIRST_LOAD_NO_INTERACTION policy plus canonical route/viewport expectations.
```
...
goal_card=docs/goals/GOAL-MVP-136_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_136_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-135 implementation commit 2f3ec67
```
...
```text
small package evidence-contract hardening for folded-state screenshot index metadata
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-136_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving screenshot-index folded evidence policy/route/viewport fields
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to screenshot-index archive evidence tightening
```
...
commit message: feat(secupilot): GOAL-MVP-136 folded screenshot index evidence
