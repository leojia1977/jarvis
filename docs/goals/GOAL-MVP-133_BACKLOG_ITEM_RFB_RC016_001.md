# GOAL-MVP-133 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-133_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Add a canonical folded-state expected-state contract into archive evidence so first-load screenshot proof can explicitly assert collapsed state in both manifest and screenshot index artifacts.
```
...
goal_card=docs/goals/GOAL-MVP-133_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_133_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-132 implementation commit 65bc42d
```
...
```text
small package evidence-contract hardening for folded-state expected-state assertion
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-133_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded-state expected-state contract
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to folded-state expected-state proof
```
...
commit message: feat(secupilot): GOAL-MVP-133 folded screenshot expected state
