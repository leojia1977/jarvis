# GOAL-MVP-130 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-130_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Add explicit first-load folded-state archive evidence metadata into SCREENSHOT_INDEX so reviewers can deterministically verify the AI advice source section was captured in folded state without interaction.
```
...
goal_card=docs/goals/GOAL-MVP-130_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_130_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-129 implementation commit 9457bf6
```
...
```text
small package evidence-contract hardening for first-load folded-state screenshot metadata
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-130_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded screenshot archive_evidence metadata
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to first-load folded-state metadata evidence
```
...
commit message: feat(secupilot): GOAL-MVP-130 folded screenshot archive metadata
