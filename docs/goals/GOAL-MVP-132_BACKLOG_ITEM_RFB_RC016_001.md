# GOAL-MVP-132 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-132_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Add a canonical folded-state assertion code into archive evidence so follow-up backlog closeout can reference one deterministic proof identifier.
```
...
goal_card=docs/goals/GOAL-MVP-132_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_132_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-131 implementation commit 1006f93
```
...
```text
small package evidence-contract hardening for folded-state assertion code
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-132_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded-state assertion code contract
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to canonical folded-state assertion code
```
...
commit message: feat(secupilot): GOAL-MVP-132 folded screenshot assertion code
