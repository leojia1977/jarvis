# GOAL-MVP-139 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-139_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Mark first-load folded screenshot archive evidence as a required pre-submission gate so RC review packages encode the requirement as executable metadata.
```
...
goal_card=docs/goals/GOAL-MVP-139_BACKLOG_ITEM_RFB_RC016_001.md
closeout=docs/S6_FAST_MVP_GOAL_MVP_139_BACKLOG_ITEM_RFB_RC016_001_2026_05_09.md
```
...
GOAL-MVP-138 implementation commit dea6b7a
```
...
```text
small package evidence-contract hardening for folded-state submission gate metadata
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-139_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded screenshot submission gate metadata exists in package manifest/package index/screenshot index
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to pre-submission gate hardening
```
...
commit message: feat(secupilot): GOAL-MVP-139 folded screenshot submission gate
