# GOAL-MVP-143 Backlog Item RFB-RC016-001

## Goal ID

```text
GOAL-MVP-143_BACKLOG_ITEM_RFB_RC016_001
```
...
```text
Add explicit folded-state review step-order metadata so reviewers can trace the screenshot check to the required review sequence.
```
...
goal_card=docs/goals/GOAL-MVP-143_BACKLOG_ITEM_RFB_RC016_001.md
...
GOAL-MVP-142 implementation commit 2e6bfdb
```
...
```text
small package evidence-contract hardening for folded-state review step-order metadata
...
```text
py -3 scripts/validate_codex_goal_card.py docs/goals/GOAL-MVP-143_BACKLOG_ITEM_RFB_RC016_001.md
...
unittest PASS output proving folded screenshot review step-order metadata exists in package manifest/package index/screenshot index
builder help command PASS output
diff --check PASS output
closeout record mapping RFB-RC016-001 to review step-order hardening
```
...
commit message: feat(secupilot): GOAL-MVP-143 folded screenshot review step
