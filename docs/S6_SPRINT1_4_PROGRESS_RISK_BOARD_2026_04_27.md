...
| State | Count | Meaning |
| --- | ---: | --- |
| Done | 16 | Repo implementation or no-code reconciliation is already accepted. |
| Running | 0 | In the active runner queue. |
| Auto-ready | 1 | Can start automatically after an immediate dependency closes. |
| Skeleton-ready | 5 | Authorized for semantic skeleton only if each checklist returns `GO`. |
...
| --- | --- | --- |
| _None_ | _N/A_ | Staged skeleton lane is active; next action is `IN-T04` skeleton launch. |
...
```text
SH-T07 reconciliation decision -> IN-T04 skeleton -> EP-T02 skeleton -> EP-T03 skeleton -> SH-T01 skeleton -> CH-T01 skeleton -> GS-T05 regression checklist
```
