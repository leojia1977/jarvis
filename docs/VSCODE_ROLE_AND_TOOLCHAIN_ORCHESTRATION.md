...
| Status | Draft docs-only role clarification |
...
| Claude Code via `cc switch` | Remains HOLD for non-interactive review automation until a callable review-only command/API path is governed and verified. `claude.exe` is not a substitute. |
...
It does not raise VS Code maturity above L1. It does not verify a new VS Code automation path. It does not change AHQ-020, AHQ-021, AHQ-022, or AHQ-019. It does not claim full four-tool automation.
