...
describe("SecuPilot first-batch workbench slice", () => {
  ...
  it("opens the S1 artifact viewer from the workbench without action controls", async () => {
    ...
    expect(reviewPanel).toHaveAttribute("data-review-scope", "LOCAL_OFFLINE_TRIAL_RC_020_CN");
    expect(reviewPanel).toHaveAttribute("data-source-candidate", "LOCAL_OFFLINE_TRIAL_RC_019_CN");
    ...
    expect(screen.getByTestId("s1-review-zip-name")).toHaveTextContent(
      "local-offline-trial-rc-020-cn-review-package-20260508.zip"
    );
    ...
    expect(screen.getByTestId("s1-run-next-step-code")).toHaveTextContent(
      "RC020_PRIVATE_PREVIEW_RC_PACKAGE_REVIEW"
    );
  ...
  it("opens the S1 local offline trial walkthrough without external authority", async () => {
    ...
    expect(screen.getByText("这起事件该怎么处理")).toBeInTheDocument();
    ...
    expect(qwenContract).toHaveAttribute("data-active-provider-mode", "qwen-synthetic-stub-ready");
    expect(qwenContract).toHaveAttribute("data-live-call-allowed", "false");
    expect(qwenContract).toHaveAttribute("data-provider-stub-ready", "true");
    expect(qwenContract).toHaveAttribute(
      "data-provider-stub-status",
  ...
  it("renders the product incident page as conclusion-first with folded evidence", async () => {
    ...
    const qwenProviderPreview = screen.getByTestId("incident-qwen-provider-dry-run");
    expect(qwenProviderPreview).toHaveAttribute("data-live-qwen-api", "false");
    expect(qwenProviderPreview).toHaveAttribute("data-network-request", "false");
    expect(qwenProviderPreview).toHaveAttribute("data-provider-stub-ready", "true");
    expect(qwenProviderPreview).toHaveAttribute(
      "data-provider-stub-status",
      "QWEN_SYNTHETIC_PROVIDER_STUB_READY_NO_NETWORK"
    );
    ...
    expect(qwenPreviewRecord).toHaveTextContent(
    ...
    expect(qwenPreviewRecord).toHaveTextContent('"network_request": false');
    expect(qwenPreviewRecord).toHaveTextContent('"api_key_required": false');
    expect(qwenPreviewRecord).toHaveTextContent('"real_data": false');
    expect(qwenPreviewRecord).toHaveTextContent('"autonomous_qwen_action": false');
